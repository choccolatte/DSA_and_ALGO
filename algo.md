...
- during merginv of two sub-arrays, the worst case scenario that generates the most comparisons, is if the sub-arrays are equally big. Just consider merging[1, 4, 6, 9] and [2, 3, 7, 8] in an array of 1-9. In this case, the following comparisons are needed - 
...
- for simplicity, lets say that we need n comparisons instead of n - 1 when merging n values. This is an ok assumption when n is large and we want to calculate an upper bound using Big O Notation.
- so, at each level merging happens, n comparisons are needed, but how many levels are there? Well, for n = 16, we ahve n = 16 = 2 pow 4, so 4 levels of merging. FOr n = 32 = 2 pow 5 there are 5 levels of merging, and at each level, n comparisons are needed. FOr n = 1024 = 2 pow 10 levels of merging is needed. To find out what power of 2 gives us 1024, we use a base-2 logarithm. The answer is 10. Mathematically it looks like this: log2 1024 = 10.

- as we can see, n comparisons are needed on each level, and there are log2n levels, so there are n.log2n comparison operations in total.
- time complexity can be calculated based on the number of split operations and the number of merge operations:
    - O((n-1) + n.log2n) = O(n.log2n)
...
- Merge sort's time complexity is linear, going diagonally between x and y axis.
...
- to find the time complexity for linear search, lets see if we can find out how many compare operations are needed to find a value in an array with n values.
...
- in graphical representation, it will look like a diagonal line from x,y to x,y(n).
...
- the implementation below is basically the same as the previous code example of encoding, but with an additional function for decoding the Huffman code.
- the `huffman_decoding` funciton takes the Huffman code, and the `codes` Python dictionary (a hashmap) with the characters and their corresponding binary codes. The function then reverses the mapping, and checks the Huffman code bit-by-bit to recreate the original text.

- example in python - Huffman code
...
- teh tabulation technique is not recursive, and it is called 'bottom-up' because of the way the final solution is built up by solving the most basic subproblems first. Sinice the most basic subproblem solutions are stored in the table first, when solving a subproblem later that relies on previous subproblems, the algorithm can just pick these solutions right from the table, no need to compute them again.
- to get a better sense of how memoization works, and is considered 'top-down', and how tabulation works, and is 'bottom-up', read this example - 
    - the bottom up tabulation approach to finding the 10th FIbonacci number works from bottom to top. We solve the last subproblem, then move to the other smaller subproblems.
    - the tabulation approach starts at the bottom by solving the F(0) first.

    - the top-down memoization approach to finding the 10th Fibonacci number works from top down, so we solve at each way, and go along storing  the solutions to subproblems we solve recursively.
    - the memoization approach starts at the top with F(10) and breaking it into smaller and smaller subproblems from there we move down.
